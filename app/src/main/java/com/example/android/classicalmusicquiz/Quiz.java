...
class Quiz {
    ...
    private Quiz(int sampleID, String composer, String title, String uri, String albumArtID) {
        mSampleID = sampleID;
        mComposer = composer;
        mTitle = title;
        mUri = uri;
    ...
    static Bitmap getComposerArtBySampleID(Context context, int sampleID){
        Quiz quiz = Quiz.getSampleByID(context, sampleID);
        int albumArtID = context.getResources().getIdentifier(
                quiz != null ? quiz.getAlbumArtID() : null, "drawable",
     ...
     */
    static Quiz getSampleByID(Context context, int sampleID) {
        JsonReader reader;
        try {
            reader = readJSONFile(context);
            reader.beginArray();
            while (reader.hasNext()) {
                Quiz currentQuiz = readEntry(reader);
                if (currentQuiz.getSampleID() == sampleID) {
                    reader.close();
                    return currentQuiz;
                }
            }
     ...
     * @return The Quiz the JsonReader is pointing to.
     */
    private static Quiz readEntry(JsonReader reader) {
        Integer id = -1;
        ...
        return new Quiz(id, composer, title, uri, albumArtID);
    }
